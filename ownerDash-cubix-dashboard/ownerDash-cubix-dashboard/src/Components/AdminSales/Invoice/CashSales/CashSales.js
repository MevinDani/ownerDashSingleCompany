...
const CashSales = () => {
        ...
        if (targetDeptNo === 'all') {
            ...
            const fetchData = async () => {
                try {
                    ...
                    const result = await fetch(`https://cubixweberp.com:199/api/SalesReport/SalesAnalysis?cmpcode=${cmpcode}&guid=${privatek}&mod=SALESLIST_CASH&deptno=NA&dfrom=${fromDate}&dto=${toDate}&salesperson=NA&group=NA&sgroup=NA&categ=NA&scateg=NA&area=NA&country=NA`)
    ...
    const handleReset = async (e) => {
        ...
        try {
            // const result = await fetch(`https://api-eproc.premierauto.ae/api/DeliveryOrderReport/DoReport/DOPending/Item?dateStart=${formattedDate}&dateEnd=${formattedDate}`)
            // const result = await fetch(`https://api-eproc.premierauto.ae/api/InvoiceSearch`)
            const result = await fetch(`https://cubixweberp.com:199/api/SalesReport/SalesAnalysis?cmpcode=${cmpcode}&guid=${privatek}&mod=SALESLIST_CASH&deptno=NA&dfrom=${formattedDate}&dto=${formattedDate}&salesperson=NA&group=NA&sgroup=NA&categ=NA&scateg=NA&area=NA&country=NA`)
            const data = await result.json()
            data && setAllData(data)
            ...
            <div className='OverallWrapper'>
                ...
                <div className='DelOContainer' style={{ paddingTop: '12px' }}>
                    ...
                    <div className='grpDateCont'>
                        <div className='grpSdText'><span><>Cash Sales</></span></div>
                                            ...
                                            displayData && displayData.map((g, i) => (
                                                <React.Fragment key={i}>
                                                    <tr onClick={window.innerWidth <= 1124 ? () => handleRowClick(i) : null} key={i}>
                                                        ...
                                                        {/* <td className='rAlign'>{g["NETCASH SALES"] === 0 ? '0' : g["NETCASH SALES"] || "Nil"}</td> */}
                                                        ...
                                                        {/* <td className='rAlign'>{g["DO NO"] === 0 ? '0' : g["DO NO"] || "Nil"}</td> */}
                                                        <td className='rAlign mobHideth'>{g["SALES MAN"] === 0 ? '0' : g["SALES MAN"] || "Nil"}</td>
                                                        {/* <td className='rAlign'>{g["NETCREDIT SALESRETURN"] === 0 ? '0' : g["NETCREDIT SALESRETURN"] || "Nil"}</td> */}
                                                        < td className='rAlign mobHideth'>{g["AMOUNT"] === 0 ? '0' : g["AMOUNT"] || "Nil"}</td>
                                                        <td td className='rAlign mobHideth'>{g["CASHCRED"] === 0 ? '0' : g["CASHCRED"] || "Nil"}</td>
                                                        <td td className='rAlign mobHideth'>{g["ACCOUNT"] === 0 ? '0' : g["ACCOUNT"] || "Nil"}</td>
                                                        <td td className='rAlign mobHideth'>{g["VATAMT"] === 0 ? '0' : g["VATAMT"] || "Nil"}</td>
                                                    ...
                                                    <td  className={hideonMob ? 'hidden' : 'expandable'}>{g["SALES MAN"] ? g["SALES MAN"] : "Nil"}</td> */}
                                                        <td td className='rAlign mobHideth' > {g["DEPTNO"] === 0 ? '0' : g["DEPTNO"] || "Nil"}</td>
                                                    </tr>
