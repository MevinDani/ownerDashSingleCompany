...
const SalesManSales2 = () => {
    ...
    const searchGrp = async () => {
        ...
        try {
            ...
            const result = await fetch(`https://cubixweberp.com:199/api/SalesReport/SalesAnalysis?cmpcode=${cmpcode}&guid=${privatek}&mod=SALES_SALESMAN&deptno=NA&dfrom=${fromDate}&dto=${toDate}&salesperson=NA&group=NA&sgroup=NA&categ=NA&scateg=NA&area=NA&country=NA`)
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const result = await fetch(`https://cubixweberp.com:199/api/SalesReport/SalesAnalysis?cmpcode=${cmpcode}&guid=${privatek}&mod=SALES_SALESMAN&deptno=NA&dfrom=${formattedDate}&dto=${formattedDate}&salesperson=NA&group=NA&sgroup=NA&categ=NA&scateg=NA&area=NA&country=NA`)
                const data = await result.json()
                data && setAllData(data)
                setShowLoader(false)
            } catch (error) {
            ...
            <div className='OverallWrapper'>
                ...
                <div className='grpContainer itemWiseSalesGrp' style={{ paddingTop: '12px' }}>
                    ...
                    <div className='grpDateCont'>
                        <div className='grpSdText'><span><>SalesMan Wise Sales</></span></div>
                                        ...
                                        {displayData && displayData.map((g, i) => (
                                            <React.Fragment key={i}>
                                                ...
                                                <td id='numD' className={expandAll ? 'expandable' : 'hidden'}>{g["NETCREDIT SALESRETURN"] ? parseFloat(g["NETCREDIT SALESRETURN"]).toFixed(2) : "Nil"}</td> */}
                                                    ...
                                                    {/* <td>{g["CUSTOMER"] ? (g["CUSTOMER"]) : "Nil"}</td> */}
                                                ...
                                                {clickedIndex === i && (
                                                    <td colSpan={2}>
                                                        ...
                                                        <tr className='mobTableDrop' style={{ backgroundColor: 'black', color: 'white' }}>
                                                            ...
                                                            <td colSpan={1} style={{ backgroundColor: 'black', color: 'white', fontSize: "16px" }} className='rAlign'>{g["SALES RETURN"] ? parseFloat(g["SALES RETURN"]).toFixed(2) : 0}</td>
                                                        </tr>
                                                        <tr className='mobTableDrop' style={{ backgroundColor: 'black', color: 'white' }}>
