...
const CustomerWiseSales = () => {
    ...
    const searchGrp = async () => {
        ...
        try {
            ...
            const result = await fetch(`https://cubixweberp.com:199/api/SalesReport/SalesAnalysis?cmpcode=${cmpcode}&guid=${privatek}&mod=SALES_CUSTOMER&deptno=NA&dfrom=${fromDate}&dto=${toDate}&salesperson=NA&group=NA&sgroup=NA&categ=NA&scateg=NA&area=NA&country=NA`)
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const result = await fetch(`https://cubixweberp.com:199/api/SalesReport/SalesAnalysis?cmpcode=${cmpcode}&guid=${privatek}&mod=SALES_CUSTOMER&deptno=NA&dfrom=${formattedDate}&dto=${formattedDate}&salesperson=NA&group=NA&sgroup=NA&categ=NA&scateg=NA&area=NA&country=NA`)
                const data = await result.json()
                data && setAllData(data)
                setShowLoader(false)
            } catch (error) {
            ...
            <div className='OverallWrapper'>
                ...
                <div className='grpContainer itemWiseSalesGrp' style={{ paddingTop: '12px' }}>
                    ...
                    <div className='grpDateCont'>
                        <div className='grpSdText'><span><>Customer Wise Sales</></span></div>
                                        ...
                                        {displayData && displayData.map((g, i) => (
                                            <React.Fragment key={i}>
                                                <tr data-testid={`dropdown-clicker`} onClick={window.innerWidth <= 768 ? () => handleRowClick(i) : null}>
                                                    <td className='rAlign mobHideth' onClick={() => handleRowClick2(i)} style={{ cursor: 'pointer' }}>
                                                        {g.ACCOUNT || 'Unknown'}
                                                        {clickedIndex === i ? (
                                                            <FaMinusCircle />
                                                        ) : (
                                                ...
                                                <td id='numD' className={expandAll ? 'expandable' : 'hidden'}>{g["NETCREDIT SALESRETURN"] ? parseFloat(g["NETCREDIT SALESRETURN"]).toFixed(2) : "Nil"}</td> */}
                                                    ...
                                                    <td onClick={() => handleRowClick2(i)} style={{ cursor: 'pointer' }}>
                                                        {clickedIndex === i ? (
                                                            <FaMinusCircle className='mobCustomerPlusMinus' />
                                                        ) : (
                                                            <FaPlusCircle className='mobCustomerPlusMinus' />
                                                        )}
                                                        {g["CUSTOMER"] ? (g["CUSTOMER"]) : "Nil"}
                                                    </td>
                                                    ...
                                                    <td id='numD' className='last rAlign mobHideth'>{g["NET SALES EXCLVAT"] ? parseFloat(g["NET SALES EXCLVAT"]).toFixed(2) : 0}</td>
                                                </tr>

                                                {clickedIndex === i && (
                                                    // <tr style={{ width: '100%' }}>

                                                    <td colspan="13" style={{ backgroundColor: "white" }} className='pcTableDrop'>
                                                ...
                                                {clickedIndex === i && (
                                                    <td colSpan={2}>
                                                        ...
                                                        <tr className='mobTableDrop' style={{ backgroundColor: 'black', color: 'white' }}>
                                                            ...
                                                            <td colSpan={1} style={{ backgroundColor: 'black', color: 'white', fontSize: "16px" }} className='rAlign'>{g.COMMISSION || '0'}</td>
                                                        </tr>
                                                        <tr className='mobTableDrop' style={{ backgroundColor: 'black', color: 'white' }}>
                                                            <td colSpan={1} style={{ backgroundColor: 'black', color: 'white', textAlign: 'left', fontSize: "16px" }}>RETURN%</td>
                                                            <td colSpan={1} style={{ backgroundColor: 'black', color: 'white', fontSize: "16px" }} className='rAlign'>{g['RETURN%'] || '0'}</td>
                                                        </tr>
                                                        <tr className='mobTableDrop' style={{ backgroundColor: 'black', color: 'white' }}>
                                                            <td colSpan={1} style={{ backgroundColor: 'black', color: 'white', textAlign: 'left', fontSize: "16px" }}>SALES RETURN</td>
                                                            <td colSpan={1} style={{ backgroundColor: 'black', color: 'white', fontSize: "16px" }} className='rAlign'>{g['SALES RETURN'] || '0'}</td>
                                                        </tr>
                                                        <tr className='mobTableDrop' style={{ backgroundColor: 'black', color: 'white' }}>
                                                            <td colSpan={1} style={{ backgroundColor: 'black', color: 'white', textAlign: 'left', fontSize: "16px" }}>VAT AMT</td>
                                                            <td colSpan={1} style={{ backgroundColor: 'black', color: 'white', fontSize: "16px" }} className='rAlign'>{g['VAT AMT'] || '0'}</td>
                                                        </tr>
