...
const StatementReport2 = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            if (searchTerm !== '') {
                try {
                    const response = await fetch(`https://cubixweberp.com:199/api/AccountSearch/MasterSearch?cmpcode=AQUA&guid=0398C380-EE63-456F-AC4F-D4AC4BDFCFBA&mod=ACCMAST_SER&deptno=NA&searchKey=${searchTerm}`);
        ...
        const fetchData = async () => {
            if (selecedtDrop !== '') {
                try {
                    ...
                    // const result = await fetch(`https://cubixweberp.com:199/api/SalesReport/SalesAnalysis?cmpcode=${cmpcode}&guid=${privatek}&mod=SALESLIST&deptno=NA&dfrom=${formattedDate}&dto=${formattedDate}&salesperson=NA&group=NA&sgroup=NA&categ=NA&scateg=NA&area=NA&country=NA`)
                    const result = await fetch(`https://cubixweberp.com:199/api/Statement/Statement?cmpcode=AQUA&guid=34B1506C-1828-4195-BDD6-28A8C9AAB007&mod=STMT_ACC1&deptno=NA&dfrom=2-FEB-2024&dto=2-FEB-2024&accounts=${selecedtDrop}&salesperson=NA`)
                    const data = await result.json()
                    data && setAllData(data)
    ...
    const [itemUiData, setItemUiData] = useState(null)
