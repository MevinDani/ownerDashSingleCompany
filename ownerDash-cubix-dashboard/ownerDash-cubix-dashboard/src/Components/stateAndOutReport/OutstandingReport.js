        ...
        const fetchData = async () => {
            if (selecedtDrop !== '') {
                try {
                    ...
                    // const result = await fetch(`https://cubixweberp.com:199/api/SalesReport/SalesAnalysis?cmpcode=${cmpcode}&guid=${privatek}&mod=SALESLIST&deptno=NA&dfrom=${formattedDate}&dto=${formattedDate}&salesperson=NA&group=NA&sgroup=NA&categ=NA&scateg=NA&area=NA&country=NA`)
                    const result = await fetch(`https://cubixweberp.com:199/api/Statement/Outstanding?cmpcode=${cmpcode}&guid=${privatek}&mod=OUT_ACC2&deptno=NA&dfrom=2-FEB-2023&dto=2-FEB-2024&accounts=${selecedtDrop}&salesperson=NA`)
                    const data = await result.json()
                    data && setAllData(data)
                ...
                <div className='DelOContainer itemWiseSalesGrp' style={{ paddingTop: '12px' }}>
                    ...
                    <div className='grpDateCont'>
                        <div className='grpSdText' style={{ justifyContent: "space-between", position: 'relative' }}>
                            <span><>Outstanding Account Report</></span>
                            <div>
                            ...
                            <table border="1" className='table table-striped table-responsive'>
                                <thead>
                                    <tr onClick={handleRowClick}>
                                        <th style={{ textAlign: 'left' }}>INVDATE</th>
                                        ...
                                        <th className='rAlign'>INV</th>
                                        <th className='lAlign'>ACCOUNT</th>
                                        <th className='lAlign'>NAME</th>
                                ...
                                <tbody>
                                            ...
                                            <React.Fragment key={i}>
                                                <tr >
                                                    <td style={{ cursor: 'pointer', textAlign: 'left' }}>
                                                        {g["INVDATE"] ? new Date(g["INVDATE"] + 'Z').toISOString().split('T')[0] :
                                                            "Nil"}
                                    ...
                                    }
                                    {
                                        ...
                                        (<tr >
                                            <td colSpan={8} style={{ color: "red" }}>Search Customer</td>
                                        </tr>)
                                ...
                                <tfoot style={{ backgroundColor: 'black', color: 'white' }}>
                                    <tr style={{ backgroundColor: 'black', color: 'white' }}>
                                        <td colSpan={5} style={{ textAlign: 'left', backgroundColor: 'black', color: 'white', fontSize: "16px" }}>Total Amount</td>
