...
const IssuedPdcTable = () => {
    ...
    const searchGrp = async () => {
        ...
        try {
            ...
            const result = await fetch(`https://cubixweberp.com:199/api/Dashboard/DashBoard?cmpcode=${cmpcode}&guid=${privatek}&mod=TOTAL_PDCI&s1=%27test%27&s2=%27test%27&s3=%27test%27&i1=100&i2=0&dt1=${formattedDate}&dt2=${toDate}`)
    ...
    const handleReset = async (e) => {
        ...
        try {
            const result = await fetch(`https://cubixweberp.com:199/api/Dashboard/DashBoard?cmpcode=${cmpcode}&guid=${privatek}&mod=TOTAL_PDCI&s1=%27test%27&s2=%27test%27&s3=%27test%27&i1=100&i2=0&dt1=${formattedDate}&dt2=2050-01-01`)
            const data = await result.json()
            data && setAllData(data)
            setShowLoader(false)
            // toOptionRef.current.value = "all"
    ...
    useEffect(() => {
        ...
        const url = `https://cubixweberp.com:199/api/Dashboard/DashBoard?cmpcode=${cmpcode}&guid=${privatek}&mod=TOTAL_PDCI&s1=%27test%27&s2=%27test%27&s3=%27test%27&i1=100&i2=0&dt1=${formattedDateToday}&dt2=2050-01-01`;
        // console.log(url)
        fetch(url)
    ...
    const totalAmount = allData && allData[0] !== 'no data available' && allData.reduce((acc, item) => acc + item.Amount, 0);
            ...
            <div className='OverallWrapper'>
                ...
                <div className='grpContainer' style={{ paddingTop: '12px' }}>
                    ...
                    <div className='grpDateCont'>
                        <div className='grpSdText'><span><>Total Issued PDC</></span></div>
                                ...
                                <table border="1" className='table table-striped table-responsive pdcTable'>
                                    ...
                                    <thead>
                                        ...
                                        <th className={expandAll ? 'expandable' : 'hidden'}>VAT AMT</th> */}
                                            ...
                                            <th scope="col" className='' onClick={handleSort} style={{ cursor: 'pointer' }}>
                                                ChqDate {sortOrder === 'asc' ? '↑' : '↓'}
                                            </th>
                                            <th className='rAlign mobHideth'>Chqno</th>
                                            <th>Supplier</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        {totalAmount ? allData && allData.map((g, i) => (
                                            <React.Fragment key={i}>
                                                ...
                                                <td id='numD' className='last'>{g["Supplier"] ? parseFloat(g["Supplier"]).toFixed(2) : "Nil"}</td> */}

                                                    <td className='rAlign mobHideth'>{g["Amount"] ? parseFloat(g["Amount"]).toFixed(2) : "Nil"}</td>
                                                    <td className='mobHideth'>{g["Bank"] ? (g["Bank"]) : "Nil"}</td>
                                                    {/* <td className='rAlign'>{g["ChqDate"] ? parseFloat(g["ChqDate"]).toFixed(2) : "Nil"}</td> */}
                                                    ...
                                                    <td className='last'>{g["Supplier"] ? (g["Supplier"]) : "Nil"}</td>
                                                </tr>
